...
for i, (train_index, test_index) in enumerate(kf.split(X), 1):
    ...
    # Inicializar variables para el bucle de entrenamiento
    threshold_mse = 0.0001
    max_iterations = 1000  # Número máximo de iteraciones para evitar un bucle infinito
    current_iteration = 0
    mse = float('inf')

    # Entrenar el modelo
    while mse > threshold_mse and current_iteration < max_iterations:
        history = model.fit(X_train, Y_train, epochs=1, verbose=0)  # Entrenar solo una iteración a la vez
        mse = history.history['mse'][0]
        current_iteration += 1
        lista_history.append(mse)
